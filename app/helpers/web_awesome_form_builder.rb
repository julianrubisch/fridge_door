class WebAwesomeFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def wa_input(method, options = {})
    ...
    @template.content_tag("wa-input", nil, options.with_defaults(name:, value: object&.send(method), required: attribute_required_on_model?(object, method)))
  end
  ...
  def select(method, choices = nil, options = {}, html_options = {}, &block)
    @template.content_tag("wa-select", options_for_select(choices, object.send(method)), options.with_defaults(label: method.to_s.humanize, value: object&.send(method)).merge(name: "#{@object_name}[#{method}]"))
  end
  ...
  def week_field(method, options = {})
    wa_input(method, options.with_defaults(label: method.to_s.humanize).merge(type: "week"))
  end
